...
def writeTestResult(sheetObj,rowNo,colsNo,testResult,
    ...
    # 测试通过结果信息为绿色，失败为红色
    colorDict = {"pass":"green","failed":"red","none":"red"}
...
def dataDriverFun(dataSourceSheetObj,caseStepSheetName):
    try:
        ...
        for idx,data in enumerate(dataIsExecuteColumn[1:]):
            ...
            # 因为第一行是标题行，所以第二行开始遍历
            if data.value == "y":
                if usernameColumn[idx + 1].value is None or usernameColumn[idx + 1].value == "None":
                    usernameColumn[idx + 1].value = ""
                # print("开始登入[%s]" % usernameColumn[idx + 1].value)
                print("开始执行用例[%s]" % usernameColumn[idx + 1].value)
                requireDatas += 1
                # 定义记录执行成功步骤数变量
                successfulSteps = 0
                for step in range(2,stepRowNums+1):
                    ...
                    if operateValue and operateValue.isalpha():
                        ...
                        if len(operateValue) < 3: # 用以区分webview的情况，使用字符串导航度进行区分
                            coordinate = operateValue + str(idx+2)
                            # print("coordinate:%s" % coordinate)
                            operateValue = str(excelObj.getCellOfValue(
                    ...
                    try:
                        ...
                        # 在测试步骤sheet中写入失败信息
                        writeTestResult(stepSheetObj, step, "caseStep", "failed", errorInfo, capturePic)
